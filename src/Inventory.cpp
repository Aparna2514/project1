...
Product* Inventory::getProductStaticById(int productId) {
    static std::vector<Product> allProducts;

    // Load once per session
    if (allProducts.empty()) {
        std::ifstream file("data/inventory.txt");
        if (!file) {
            std::cerr << "Error: Cannot read inventory file.\n";
            return nullptr;
        }
        ...
        while (getline(file, line)) {
            ...
            int id, quantity, reorderThreshold;
            double price, discount, gst;
            std::string name, category, brand, description;
            ...
            allProducts.emplace_back(id, name, category, brand, description, price, quantity, discount, gst, reorderThreshold);
        }
    }
